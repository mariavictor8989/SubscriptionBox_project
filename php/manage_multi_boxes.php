...
// Get all subscriptions with their latest box
$subs = $conn->prepare("
    SELECT us.id, us.status, us.next_billing,
           sp.name AS plan_name, sp.theme, sp.tier, sp.price,
           b.id AS box_id, b.status AS box_status, b.month, b.box_type
    FROM user_subscriptions us
    JOIN subscription_plans sp ON sp.id = us.plan_id
    LEFT JOIN boxes b ON b.subscription_id = us.id
        AND b.id = (SELECT id FROM boxes WHERE subscription_id = us.id ORDER BY created_at DESC LIMIT 1)
    WHERE us.user_id = ?
...
<div class="container mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="subscribe.php" class="btn btn-success">+ Add New Box</a>
    </div>
    ...
    <div class="box-card <?= $sub['status'] ?>">
        <div class="d-flex justify-content-between align-items-start mb-3">
            ...
            <div class="d-flex gap-2 align-items-center">
                <span class="tier-pill tier-<?= $sub['tier'] ?>"><?= strtoupper($sub['tier']) ?></span>
                <?php
        ...
        <div class="row text-center mb-3">
            <div class="col">
                ...
                <div class="fw-bold"><?= $sub['price'] ?> EGP/mo</div>
            </div>
            <div class="col">
                <div class="text-muted small">Next Billing</div>
                <div class="fw-bold"><?= $sub['next_billing'] ?></div>
            </div>
            <div class="col">
                <div class="text-muted small">Box Status</div>
                <div class="fw-bold"><?= $sub['box_status'] ? ucfirst($sub['box_status']) : '—' ?></div>
            </div>
            <div class="col">
                <div class="text-muted small">Box Type</div>
                <div class="fw-bold"><?= $sub['box_type'] ? ucfirst($sub['box_type']) : '—' ?></div>
            </div>
        </div>

        <?php if ($sub['status'] === 'active'): ?>
        <div class="d-flex gap-2 flex-wrap">
            <a href="customize_box.php?sub=<?= $sub['id'] ?>" class="btn btn-success btn-sm">✏️ Customize</a>
            <a href="manage_subscription.php" class="btn btn-outline-warning btn-sm">⏸ Pause</a>
            <a href="order_box.php" class="btn btn-outline-primary btn-sm">🛒 Order</a>
        </div>
        <?php endif; ?>
    </div>
    <?php endforeach; ?>
